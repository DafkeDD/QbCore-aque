    ...
    while true do
        ...
        if inRange and not shown then
        	...
        	if Config.okokTextUI then
        		exports['okokTextUI']:Open('[E] Komma åt banken', 'darkblue', 'left')
            else
        		TriggerEvent('QBCore:Notify', "Tryck på E för att komma åt banken", "success")
        	end
...
AddEventHandler("okokBanking:OpenATM", function()
	...
	QBCore.Functions.TriggerCallback("okokBanking:GetPIN", function(pin)
		if pin then
			...
			exports['okokNotify']:Alert("BANK", "Gå upp till en banken för att ställa in en PIN-kod", 5000, 'info')
		end
	end)
end)
	...
	elseif data.action == "deposit" then
		if tonumber(data.value) ~= nil then
			if tonumber(data.value) > 0 then
				...
				exports['okokNotify']:Alert("BANK", "Ogiltigt belopp", 5000, 'error')
			end
		else
			exports['okokNotify']:Alert("BANK", "Felaktig input", 5000, 'error')
		end
			...
			if tonumber(data.value) > 0 then
				QBCore.Functions.TriggerCallback("okokBanking:IsIBanUsed", function(isUsed, isPlayer, name)
					...
					elseif isUsed == nil then
						exports['okokNotify']:Alert("BANK", "Detta kontonummeret finns inte", 5000, 'error')
					end
				end, data.iban:upper())
			else
				exports['okokNotify']:Alert("BANK", "Ogiltigt belopp", 5000, 'error')
	...
	elseif data.action == "change_iban" then
		if Config.CustomIBANAllowLetters then
			...
			QBCore.Functions.TriggerCallback("okokBanking:IsIBanUsed", function(isUsed, isPlayer)
				...
				elseif isUsed ~= nil then
					exports['okokNotify']:Alert("BANK", "Detta kontonummret används redan", 5000, 'error')
				end
			end, iban)
		elseif not Config.CustomIBANAllowLetters then
			if tonumber(data.iban) ~= nil then
				...
				exports['okokNotify']:Alert("BANK", "Du kan bara använda nummer på ditt IBAN", 5000, 'error')
			end
		end
	elseif data.action == "change_pin" then
		if tonumber(data.pin) ~= nil then
			if string.len(data.pin) == 4 then
				...
				exports['okokNotify']:Alert("BANK", "Din PIN-kod måste vara fyra siffror lång", 5000, 'info')
			end
		else
			exports['okokNotify']:Alert("BANK", "Du kan bara använda siffror", 5000, 'info')
		end
	end
end)
