		...
		if fishing then
			...
			if fishing then
				...
				if IsEntityDead(playerPed) or IsEntityInWater(playerPed) then
					endFishing()
					QBCore.Functions.Notify('Fiskning avslutad', 'error')
				end
			end
...
CreateThread(function()
	while true do
		...
		if fishing then
			...
			exports['textUi']:DrawTextUi('show', "Tryck [F] för att fånga fisk!")
			input = 0
			pausetimer = 0
		end
	end
	...
	QBCore.Functions.TriggerCallback('QBCore:HasItem', function(HasItem)
		if HasItem then
			QBCore.Functions.Progressbar("accepted_key", "Sätter in nyckel..", (math.random(2000, 5000)), false, true, {
				...
				ClearPedTasks(ped)
				QBCore.Functions.Notify("Avbrytet!", "error")
			end)
		else
		  QBCore.Functions.Notify("Du har ingen nyckel till detta lås!", "error")
		end
	  end, 'fishingkey')
end)
...
RegisterNetEvent('fishing:SkillBar', function(message)
	...
	if Config.Skillbar == "reload-skillbar" then
		...
		if finished ~= 100 then
			QBCore.Functions.Notify('Fisken kom undan!', 'error')
			loseBait()
		else
...
RegisterNetEvent('fishing:fishstart', function()
	...
	local pos = GetEntityCoords(playerPed) 
	if IsPedSwimming(playerPed) then return QBCore.Functions.Notify("Du kan inte simma och fiska samtidigt.", "error") end 
	...
	if GetWaterHeight(pos.x, pos.y, pos.z-2, pos.z - 3.0)  then
		...
		Wait(time)
		exports['textUi']:DrawTextUi('show', "Tryck [X] för att sluta fiska")
		fishAnimation()
	else
...
openedTreasureChest = function()
	...
	QBCore.Functions.Notify("Skattkistan öppnad! Var noga med att samla all din byte!!", "success", 7500)
	local ShopItems = {} 
