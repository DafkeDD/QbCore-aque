...
RegisterNetEvent('ps-hud:toggleHud')
AddEventHandler('ps-hud:toggleHud', function(toggleHud1)
        ...
        if QBCore ~= nil and isLoggedIn and PSHud.Show then
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
                ...
                if speed >= FXStress.MinimumSpeed then
                    TriggerServerEvent('ps-hud:Server:GainStress', math.random(1, 2))
                end
            end
        end
        Citizen.Wait(20000)
...
RegisterNetEvent('ps-hud:client:ToggleHarness')
AddEventHandler('ps-hud:client:ToggleHarness', function(toggle)
    SendNUIMessage({
        action = "harness",
        toggle = toggle
    })
...
RegisterNetEvent('ps-hud:client:UpdateNitrous')
AddEventHandler('ps-hud:client:UpdateNitrous', function(toggle, level, IsActive)
    SendNUIMessage({
        action = "nitrous",
        toggle = toggle,
        level = level,
...
RegisterNetEvent('ps-hud:client:UpdateDrivingMeters')
AddEventHandler('ps-hud:client:UpdateDrivingMeters', function(toggle, amount)
    SendNUIMessage({
...
RegisterNetEvent('ps-hud:toggleCompass')
AddEventHandler('ps-hud:toggleCompass', function(toggleCompass1)
    ...
    while true do
        if not IsGaining then
            ...
            if StressGain > 0 then
                ...
                TriggerServerEvent('ps-hud:Server:UpdateStress', StressGain)
                StressGain = 0
            end
        end
    ...
    while true do
        if QBCore ~= nil and isLoggedIn then
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
                speed = GetEntitySpeed(GetVehiclePedIsIn(GetPlayerPed(-1), false)) * 3.6
                if speed >= FXStress.MinimumSpeed then
                    TriggerServerEvent('ps-hud:Server:GainStress', math.random(2, 4))
                end
            end
        end
        Citizen.Wait(20000)
