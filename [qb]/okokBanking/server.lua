...
QBCore.Functions.CreateCallback("okokBanking:GetPlayerInfo", function(source, cb)
	...
	exports['oxmysql']:execute('SELECT * FROM players WHERE citizenid = @identifier', {
...
QBCore.Functions.CreateCallback("okokBanking:IsIBanUsed", function(source, cb, iban)
	...
	exports['oxmysql']:execute('SELECT * FROM players WHERE iban = @iban', {
		...
		if db ~= nil then
			...
			exports['oxmysql']:execute('SELECT * FROM okokBanking_societies WHERE iban = @iban', {
				['@iban'] = iban
			}, function(result2)
				local db2 = result2[1]
...
QBCore.Functions.CreateCallback("okokBanking:GetPIN", function(source, cb)
	...
	exports['oxmysql']:execute('SELECT pincode FROM players WHERE citizenid = @identifier', {
		['@identifier'] = xPlayer.PlayerData.citizenid,
	}, function(result)
		local pin = result[1]
...
QBCore.Functions.CreateCallback("okokBanking:SocietyInfo", function(source, cb, society)
	exports['oxmysql']:execute('SELECT * FROM okokBanking_societies WHERE society = @society', {
		['@society'] = society
	}, function(result)
		local db = result[1]
		cb(db)
...
AddEventHandler("okokBanking:CreateSocietyAccount", function(society, society_name, value, iban)
	exports['oxmysql']:execute('INSERT INTO okokBanking_societies (society, society_name, value, iban) VALUES (@society, @society_name, @value, @iban)', {
		['@society'] = society,
...
AddEventHandler("okokBanking:SetIBAN", function(iban)
	...
	exports['oxmysql']:execute('UPDATE players SET iban = @iban WHERE citizenid = @identifier', {
		['@identifier'] = xPlayer.PlayerData.citizenid,
		['@iban'] = iban,
	}, function (result)
	end)
...
AddEventHandler("okokBanking:DepositMoneyToSociety", function(amount, society, societyName)
	...
	if amount <= playerMoney then
		exports['oxmysql']:execute('UPDATE okokBanking_societies SET value = value + @value WHERE society = @society AND society_name = @society_name', {
			['@value'] = amount,
			['@society'] = society,
...
AddEventHandler("okokBanking:WithdrawMoneyToSociety", function(amount, society, societyName, societyMoney)
	...
	exports['oxmysql']:execute('UPDATE okokBanking_societies SET is_withdrawing = 1 WHERE society = @society AND society_name = @society_name', {
		['@value'] = amount,
		['@society'] = society,
		['@society_name'] = societyName,
	}, function(changed)
	...
	exports['oxmysql']:execute('UPDATE okokBanking_societies SET is_withdrawing = 0 WHERE society = @society AND society_name = @society_name', {
		['@value'] = amount,
		['@society'] = society,
		['@society_name'] = societyName,
	}, function(changed)
...
AddEventHandler("okokBanking:TransferMoneyToSociety", function(amount, ibanNumber, societyName, society)
	...
	local playerMoney = xPlayer.PlayerData.money.bank
		...
		if amount <= playerMoney then
			exports['oxmysql']:execute('UPDATE okokBanking_societies SET value = value + @value WHERE iban = @iban', {
				['@value'] = amount,
				['@iban'] = ibanNumber
			}, function(changed)
			end)
...
AddEventHandler("okokBanking:AddDepositTransaction", function(amount, source_)
	...
	exports['oxmysql']:execute('INSERT INTO okokBanking_transactions (receiver_identifier, receiver_name, sender_identifier, sender_name, date, value, type) VALUES (@receiver_identifier, @receiver_name, @sender_identifier, @sender_name, CURRENT_TIMESTAMP(), @value, @type)', {
		['@receiver_identifier'] = 'bank',
		['@receiver_name'] = 'Bankkonto',
...
AddEventHandler("okokBanking:UpdatePINDB", function(pin, amount)
	...
	if amount <= xPlayer.PlayerData.money.bank then
		exports['oxmysql']:execute('UPDATE players SET pincode = @pin WHERE citizenid = @identifier', {
			['@pin'] = pin,
			['@identifier'] = xPlayer.PlayerData.citizenid,
		}, function(changed)
		end)
