...
RegisterNetEvent('qb-gangmenu:client:OpenMenu', function()
    ...
    local gangMenu = {
        ...
        {
            header = "📋 Hantera gängmedlemmar",
            txt = "Rekrytera eller avfyra gängmedlemmar",
            params = {
                event = "qb-gangmenu:client:ManageGang",
            }
        },
        {
            header = "💛 Rekrytera medlemmar",
            txt = "Anställ gängmedlemmar",
            params = {
        ...
        },
        {
            ...
            txt = "Kontrollera din gängbalans",
            params = {
...
RegisterNetEvent('qb-gangmenu:client:ManageGang', function()
    local GangMembersMenu = {
        {
            header = "Hantera gängmedlemmar - " .. string.upper(PlayerGang.label),
            isMenuHeader = true,
        },
    }
...
RegisterNetEvent('qb-gangmenu:lient:ManageMember', function(data)
    local MemberMenu = {
        {
            header = "Hantera " .. data.player.name .. " - " .. string.upper(PlayerGang.label),
            isMenuHeader = true,
        },
    }
...
RegisterNetEvent('qb-gangmenu:client:HireMembers', function()
    local HireMembersMenu = {
        {
            header = "Anställ gängmedlemmar - " .. string.upper(PlayerGang.label),
            isMenuHeader = true,
        },
    }
        ...
        for _, v in pairs(players) do
            if v and v ~= PlayerId() then
                HireMembersMenu[#HireMembersMenu + 1] = {
                    header = v.name,
                    txt = "Medborgar ID: " .. v.citizenid .. " - ID: " .. v.sourceplayer,
                    params = {
                        isServer = true,
...
RegisterNetEvent('qb-gangmenu:client:SocietyMenu', function()
    QBCore.Functions.TriggerCallback('qb-gangmenu:server:GetAccount', function(cb)
        local SocietyMenu = {
            {
                header = "Pengar: " .. comma_valueGang(cb) .. "KR - " .. string.upper(PlayerGang.label),
                isMenuHeader = true,
            },
            {
                header = "💸 Sätt In",
...
RegisterNetEvent('qb-gangmenu:client:SocietyDeposit', function(saldoattuale)
    local deposit = exports['qb-input']:ShowInput({
        header = "Sätt in pengar <br> Tillgängligt saldo: $" .. saldoattuale,
        submitText = "Confirm",
        inputs = {
            {
                type = 'number',
...
RegisterNetEvent('qb-gangmenu:client:SocietyWithdraw', function(saldoattuale)
    local withdraw = exports['qb-input']:ShowInput({
        header = "Ta ut pengar <br> Tillgängligt saldo: $" .. saldoattuale,
        submitText = "Confirm",
        inputs = {
            {
                type = 'number',
