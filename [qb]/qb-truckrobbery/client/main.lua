...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
            ...
            if dist <= 1.0 then
				DrawText3D(MissionMarker.x, MissionMarker.y, MissionMarker.z, "~g~[E]~b~ Att acceptera uppdrag")
...
AddEventHandler('AttackTransport:InfoForLspd', function(x, y, z)
	if PlayerJob ~= nil and PlayerJob.name == 'police' then
		...
		if PoliceBlip == 0 then
			...
			AddTextComponentString('Överfall på transport av kontanter')
    ...
    if PlayerJob.name == "police" then
        local store = "Armored Truck"
            ...
            TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
                timeOut = 10000,
                alertTitle = "Bepansrad lastbilsrånförsök",
                coords = {
                    x = coords.x,
                    y = coords.y,
                    z = coords.z,
        ...
        BeginTextCommandSetBlipName('STRING')
        AddTextComponentString("10-90: Bepansrad lastbilsrånförsök")
        EndTextCommandSetBlipName(blip)
        while transG ~= 0 do
            Wait(180 * 4)
            transG = transG - 1
...
function MissionNotification()
	...
	sender = "Bossen",
	subject = "Nytt mål",
	message = "Så du är intresserad av att tjäna lite pengar? bra... skaffa dig en pistol och få det att hända... skickar dig platsen nu.",
	})
	Citizen.Wait(3000)
end
---
    ...
    while true do
		...
		if MissionStart == 1 then
			...
			if dist <= 55.0  then
				...
				if warning == 0 then
				warning = 1
				QBCore.Functions.Notify("Bli av med vakterna innan du placerar bomben.", "error")
				end
			...
			if dist <= 7 and BlownUp == 0 and PlayerJob.name ~= 'police' then
				if BlowBackdoor == 0 then
					hintToDisplay('Tryck på [G] för att spränga bakdörren och ta pengarna')
					BlowBackdoor = 1
				end
				if IsControlPressed(0, 47) and GuardsDead == 1 then
...
if IsVehicleStopped(transport) then
	if IsVehicleSeatFree(transport, -1) and IsVehicleSeatFree(transport, 0) and IsVehicleSeatFree(transport, 1) and GuardsDead == 1 then
		if not IsEntityInWater(PlayerPedId()) then
			...
			BlownUp = 1
			lootable = 1
			QBCore.Functions.Notify('Du kan börja samla in kontanter.', "success")
			RemoveBlip(TruckBlip)
		else
			QBCore.Functions.Notify('Gå upp ur vattnet', "error")
		end
	else
		QBCore.Functions.Notify('Fordonet måste vara tomt för att kunna placera lasten', "error")
	end
else
	QBCore.Functions.Notify('Du kan inte råna ett fordon som rör sig.', "error")
end
end
		...
		if lootable == 1 then
			...
			if dist <= 4.5 then
				if PickupMoney == 0 then
					hintToDisplay('Tryck på [E] för att ta pengarna')
					PickupMoney = 1
				end
				if IsControlJustPressed(0, 38) then
				lootable = 0
...
function TakingMoney()
	...
	FreezeEntityPosition(PlayerPedId(), true)
	QBCore.Functions.Notify('Du packar kontanter i en väska', "success")
	local _time = GetGameTimer()
	while GetGameTimer() - _time < 20000 do
		if IsControlPressed(0, 47) then
			break
		end
		hintToDisplay('Håll [G] för att rädda dig')
		Citizen.Wait(1)
	end
