...
RegisterNetEvent('police:client:RobPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        local playerId = GetPlayerServerId(player)
        if IsEntityPlayingAnim(playerPed, "missminuteman_1ig_2", "handsup_enter", 3) or IsEntityPlayingAnim(playerPed, "mp_arresting", "idle", 3) or IsTargetDead(playerId) then
            QBCore.Functions.Progressbar("robbing_player", "Rånar person...", math.random(5000, 7000), false, true, {
...
RegisterNetEvent('police:client:JailPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if tonumber(time) > 0 then
            ...
            QBCore.Functions.Notify("Tiden måste vara högre än 0..", "error")
        end
    else
        QBCore.Functions.Notify("Ingen i närheten!", "error")
    end
    ...
    if not IsPedRagdoll(PlayerPedId()) then
        ...
        if player ~= -1 and distance < 1.5 then
            ...
            if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(GetPlayerPed(PlayerPedId())) then
                ...
                QBCore.Functions.Notify("Du kan inte belägga någon med handklovar i bilen.", "error")
            end
        else
            QBCore.Functions.Notify("Ingen i närheten!", "error")
        end
        ...
        if player ~= -1 and distance < 1.5 then
            QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                if result then 
                    ...
                    QBCore.Functions.Notify("Du har inga handbojor på dig", "error")
                end
            end, "handcuffs")
        else
            QBCore.Functions.Notify("Ingen i närheten!", "error")
...
RegisterNetEvent('police:client:GetCuffed', function(playerId, isSoftcuff)
    ...
    if not isHandcuffed then
        ...
        if not isSoftcuff then
            ...
            QBCore.Functions.Notify("Du är fängslad!")
        else
            cuffType = 49
            GetCuffedAnimation(playerId)
            QBCore.Functions.Notify("Du är fängslad, men du kan fortfarande gå.")
        end
    else
        isHandcuffed = false
        isEscorted = false
