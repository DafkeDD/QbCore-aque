...
QBCore.Functions.CreateCallback("qb-faktura:GetInvoices", function(source, cb)
...
RegisterServerEvent("qb-faktura:PayInvoice")
AddEventHandler("qb-faktura:PayInvoice", function(invoice_id)
	local xPlayer = QBCore.Functions.GetPlayer(source)
	local _source = source
	...
	exports.oxmysql:execute('SELECT * FROM okokBilling WHERE id = @id', {
		...
		if playerMoney < invoices.invoice_value then
			...
			TriggerEvent("qb-bossmenu:server:qb-fakturaDeposit", invoices.society, invoices.invoice_value)
...
RegisterServerEvent("qb-faktura:CancelInvoice")
AddEventHandler("qb-faktura:CancelInvoice", function(invoice_id)
	local xPlayer = QBCore.Functions.GetPlayer(source)
	local _source = source
...
RegisterServerEvent("qb-faktura:CreateInvoice")
AddEventHandler("qb-faktura:CreateInvoice", function(data)
...
QBCore.Functions.CreateCallback("qb-faktura:GetSocietyInvoices", function(source, cb, society)
	local xPlayer = QBCore.Functions.GetPlayer(source)
				...
				if xPlayer == nil then
					exports.oxmysql:execute('SELECT money FROM players WHERE citizenid = @id', {
						...
						exports.oxmysql:execute('UPDATE players SET money = @playerAccount WHERE citizenid = @target', {
							...
							TriggerEvent("qb-bossmenu:server:qb-fakturaDeposit", v.society, invoice_value)
