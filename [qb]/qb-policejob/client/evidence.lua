...
local StatusList = {
    ['fight'] = 'Rodnader på händerna',
    ['widepupils'] = 'Vidgade pupiller',
    ['redeyes'] = 'Röda ögon',
    ['weedsmell'] = 'Luktar weed',
    ['gunpowder'] = 'Krut på kläderna',
    ['chemicals'] = 'Stinker kemikalier',
    ['heavybreath'] = 'Andas tungt',
    ['sweat'] = 'Svettas mycket',
    ['handbleed'] = 'Blod på händerna',
    ['confused'] = 'Förvirrad',
    ['alcohol'] = 'Luktar alkohol',
    ['heavyalcohol'] = 'Stinker sprit'
}
...
RegisterNetEvent('evidence:client:ClearBlooddropsInArea', function()
    ...
    local blooddropList = {}
    QBCore.Functions.Progressbar('clear_blooddrops', 'Städar bort allt blood..', 5000, false, true, {
        ...
        QBCore.Functions.Notify('Blodet rensades inte..', 'error')
    end)
end)
...
RegisterNetEvent('evidence:client:AddCasing', function(casingId, weapon, coords, serie)
    Casings[casingId] = {
        type = weapon,
        serie = serie and serie or 'Serienummret syns inte..',
        coords = {
            x = coords.x,
            y = coords.y,
            z = coords.z - 0.9
...
RegisterNetEvent('evidence:client:ClearCasingsInArea', function()
    local pos = GetEntityCoords(PlayerPedId())
    local casingList = {}
    QBCore.Functions.Progressbar('clear_casings', 'Ta bort kulhylsor..', 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true
        ...
        if Casings and next(Casings) then
            ...
            QBCore.Functions.Notify('Kulhylsor borttagna :)')
        end
    end, function() -- Cancel
        QBCore.Functions.Notify('Kulhylsor ej borttagna', 'error')
    end)
end)
    ...
    while true do
        ...
        if CurrentCasing and CurrentCasing ~= 0 then
            ...
            if #(pos -vector3(Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z)) < 1.5 then
                DrawText3D(Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z, '~g~G~w~ - tomhylsa ~b~#' .. Casings[CurrentCasing].type)
