...
RegisterKeyMapping('createscene', 'Skapa Scene', 'keyboard', Config.CreateSceneKey)
RegisterKeyMapping('deletescene', 'Radera Scene', 'keyboard', Config.DeleteSceneKey)
    ...
    if creationLaser then
        CreateThread(function()
            while creationLaser do
                local hit, coords = DrawLaser('Tryck ~g~E~w~ För att placera\nTryck ~g~G~w~ För att redigera scene', {r = 2, g = 241, b = 181, a = 200})
    ...
    if deletionLaser then
        CreateThread(function()
            while deletionLaser do
                local hit, coords = DrawLaser('Tryck ~r~E~w~ För att radera en scene\nTryck ~r~G~w~ För att avbryta', {r = 255, g = 0, b = 0, a = 200})
