...
RegisterNetEvent("doj:client:startPlanting",function(plant)
    ...
    if GetGroundHash(ped) == -1286696947 or GetGroundHash(ped) == -1885547121 or GetGroundHash(ped) == 223086562 or GetGroundHash(ped) == -461750719 then
        ...
        if canPlant then
            ...
            QBCore.Functions.Notify('Du ska nära en annan anläggning', 'error')
        end
    else
        QBCore.Functions.Notify('Du kan inte plantera här', 'error')
    end
end)
...
RegisterNetEvent("doj:client:addPlant",function(seed, coords, id)
    ...
    QBCore.Functions.Notify("Planterar...", "success", 6000)
...
RegisterNetEvent('doj:client:cropOptions', function(args, data)
	...
	if args == 1 then
        local percent = math.random(10,35)
        QBCore.Functions.TriggerCallback('QBCore:HasItem', function(HasItem)
            if HasItem then
                QBCore.Functions.Notify("+"..percent.." gödningsmedel", "success")
                if percent > 0 then
                ...
                PlantMenuAlive()
                QBCore.Functions.Notify("Du har ingen gödsel", "error")
            end
        end, "marijuana_nutrition")
    elseif args == 2 then
        local percent = math.random(5,45)
        QBCore.Functions.TriggerCallback('QBCore:HasItem', function(HasItem)
            if HasItem then
                QBCore.Functions.Notify("+"..percent.." Vatten", "success")
                if percent > 0 then
                ...
                PlantMenuAlive()
                QBCore.Functions.Notify("Du har inget vatten", "error")
            end
        end, "marijuana_water")
    elseif args == 3 then
        local ped = PlayerPedId()
...
function PlantMenuDead()
    exports['qb-menu']:showHeader({
        {
            header = "Död marijuanaväxt: "..CurrentPlant,
            txt = "Steg: "..math.floor(CurrentPlantInfo.stage).."%", 
            isMenuHeader = true
        },
        {
            header = "Skörda död gröda",
            txt = "",
            params = {
...
function PlantMenuStacic()
    exports['qb-menu']:openMenu({
        { 
            header = "Marijuanaväxt: "..CurrentPlant,
            txt = "Steg: "..math.floor(CurrentPlantInfo.stage).."%<p>Betyg: "..math.floor(CurrentPlantInfo.rate).."%",
            isMenuHeader = true
        },
        {
            header = "Gödselmedel",
            txt = "Näring: "..math.floor(CurrentPlantInfo.food).."%",
            params = {
                event = "doj:client:cropOptions",
                args = 1
            }
        ...
        {
            header = "Vatten",
            txt = "Vatten: "..math.floor(CurrentPlantInfo.water).."%",
            params = {
                event = "doj:client:cropOptions",
                args = 2
            }
