...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
	...
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "glovebox" then
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			...
			TriggerClientEvent("QBCore:Notify", src, "Föremål finns inte???", "error")
		end
		...
		if QBCore.Shared.SplitStr(shopType, "_")[1] == "Dealer" then
			if QBCore.Shared.SplitStr(itemData.name, "_")[1] == "weapon" then
				...
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
					...
					TriggerClientEvent('QBCore:Notify', src, itemInfo["label"] .. " köpt!", "success")
					...
					TriggerClientEvent('QBCore:Notify', src, "Du har inte tillräckligt med pengar..", "error")
				end
			else
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
	...
	elseif fromInventory == "crafting" then
		...
		if hasCraftItems(src, itemData.costs, fromAmount) then
			...
			TriggerClientEvent('QBCore:Notify', src, "Du har inte rätt föremål..", "error")
		end
	elseif fromInventory == "attachment_crafting" then
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			...
			TriggerClientEvent("QBCore:Notify", src, "Varan finns inte??", "error")
		end
	end
end)
...
RegisterServerEvent("inventory:server:GiveItem", function(target, name, amount, slot)
	...
	if Player == OtherPlayer then return TriggerClientEvent('QBCore:Notify', src, "Kan du inte ge dig själv ett föremål?") end
	if dist > 2 then return TriggerClientEvent('QBCore:Notify', src, "Du är för långt bort för att ge saker!") end
	local item = Player.Functions.GetItemBySlot(slot)
	if not item then TriggerClientEvent('QBCore:Notify', src, "Det gick inte att hitta föremålet du försökte ge!"); return end
	if item.name ~= name then TriggerClientEvent('QBCore:Notify', src, "Felaktig artikel hittades försök igen!"); return end
	...
	if amount <= item.amount then
		...
		if Player.Functions.RemoveItem(item.name, amount, item.slot) then
			if OtherPlayer.Functions.AddItem(item.name, amount, false, item.info) then
				...
				TriggerClientEvent('QBCore:Notify', target, "Du Fick "..amount..' '..item.label.." Från "..Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname)
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, true)
				TriggerClientEvent('inventory:client:ItemBox',src, QBCore.Shared.Items[item.name], "remove")
				TriggerClientEvent('QBCore:Notify', src, "Du Gav " .. OtherPlayer.PlayerData.charinfo.firstname.." "..OtherPlayer.PlayerData.charinfo.lastname.. " " .. amount .. " " .. item.label .."!")
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, true)
				TriggerClientEvent('qb-inventory:client:giveAnim', src)
				TriggerClientEvent('qb-inventory:client:giveAnim', target)
			else
				Player.Functions.AddItem(item.name, amount, item.slot, item.info)
				TriggerClientEvent('QBCore:Notify', src,  "Andra Spelarens Inventory Är Fullt", "error")
				TriggerClientEvent('QBCore:Notify', target,  "Ditt inventory är fullt!", "error")
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, false)
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, false)
			end
		else
			TriggerClientEvent('QBCore:Notify', src,  "Du har inte tillräckligt med föremålet", "error")
		end
	else
		TriggerClientEvent('QBCore:Notify', src, "Du har inte tillräckligt med objekt att överföra")
	end
end)
...
QBCore.Commands.Add("resetinv", "Återställ inventory (Endast Admin)", {{name="type", help="stash/trunk/glovebox"},{name="id/plate", help="ID för stash eller registreringsskylt"}}, true, function(source, args)
...
QBCore.Commands.Add("rob", "Råna Spelare", {}, false, function(source, args)
	TriggerClientEvent("police:client:RobPlayer", source)
end)

QBCore.Commands.Add("giveitem", "Ge ett objekt (Endast Administratör)", {{name="id", help="Player ID"},{name="item", help="Objektets namn (inte en etikett)"}, {name="amount", help="Amount of items"}}, true, function(source, args)
		...
		if amount > 0 then
			if itemData then
				...
				if Player.Functions.AddItem(itemData["name"], amount, false, info) then
					TriggerClientEvent('QBCore:Notify', source, "Du gav" ..GetPlayerName(tonumber(args[1])).." "..amount.." "..itemData["name"].. "", "success")
				else
					TriggerClientEvent('QBCore:Notify', source,  "Kan inte ge föremål!", "error")
				end
			else
				TriggerClientEvent('QBCore:Notify', source,  "Objektet finns inte", "error")
			end
		else
			TriggerClientEvent('QBCore:Notify', source,  "Ogiltigt belopp", "error")
		end
	else
		TriggerClientEvent('QBCore:Notify', source,  "Spelaren är inte online", "error")
	end
end, "admin")

QBCore.Commands.Add("randomitems", "Ge slumpmässiga föremål (Endast God)", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
	local filteredItems = {}
