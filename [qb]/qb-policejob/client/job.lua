...
function TakeOutVehicle(vehicleInfo)
    ...
    QBCore.Functions.SpawnVehicle(vehicleInfo, function(veh)
	    SetCarItemsInfo()
        SetVehicleNumberPlateText(veh, "POLIS"..tostring(math.random(1000, 9999)))
        SetEntityHeading(veh, coords.w)
...
function MenuImpound(currentSelection)
    ...
    QBCore.Functions.TriggerCallback("police:GetImpoundedVehicles", function(result)
        ...
        if result == nil then
            QBCore.Functions.Notify("Det finns inga beslagtagna fordon", "error", 5000)
        else
            shouldContinue = true
            for _ , v in pairs(result) do
                ...
                impoundMenu[#impoundMenu+1] = {
                    ...
                    txt = "Motor: " .. enginePercent .. "% | Bränsle: "..currentFuel.. "%",
                    params = {
...
RegisterNetEvent('police:client:SendEmergencyMessage', function()
    TriggerServerEvent('police:server:policeAlert', 'PRIO 1 - Blåljuspersonal i fara. Polis eller Sjukvård')
...
RegisterNetEvent('police:client:EvidenceStashDrawer', function(data)
    ...
    if #(pos - takeLoc) <= 1.0 then
        local drawer = exports['qb-input']:ShowInput({
            header = 'Bevis Förråd | '.. currentEvidence,
            submitText = "open",
            inputs = {
                {
                    type = 'number',
        ...
        if drawer then
            ...
            TriggerServerEvent("inventory:server:OpenInventory", "stash", currentEvidence.." | Låda "..drawer.slot, {
                maxweight = 4000000,
                slots = 500,
            })
            TriggerEvent("inventory:client:SetCurrentStash", currentEvidence.." | Låda "..drawer.slot)
        end
    else
        exports['qb-menu']:closeMenu()
    end
        ...
        if LocalPlayer.state.isLoggedIn and PlayerJob.name == "police" then
            ...
            for k, v in pairs(Config.Locations["duty"]) do
                if #(pos - v) < 9 then
                    ...
                    if #(pos - v) < 4 then
                        if not onDuty then
                            DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Stämpla in")
                        else
                            DrawText3D(v.x, v.y, v.z, "~r~E~w~ - Stämpla ut")
                        end
                    ...
                    elseif #(pos - v) < 8 then
                        DrawText3D(v.x, v.y, v.z, "Stämpla In/Ut")
                    end
                end
            end
        end
            ...
            for k, v in pairs(Config.Locations["evidence"]) do
                if #(pos - v) < 2 then
                    ...
                    elseif #(pos - v) < 1.5 then
                        DrawText3D(v.x, v.y, v.z, "Förråd " .. k)
                        if headerDrawn then
                            headerDrawn = false
                            exports['qb-menu']:closeMenu()
                        end
            ...
            for k, v in pairs(Config.Locations["stash"]) do
                if #(pos - v) < 4.5 then
                    if onDuty then
                        sleep = 5
                        if #(pos - v) < 1.5 then
                            DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Personal Förråd")
                            if IsControlJustReleased(0, 38) then
                        ...
                        elseif #(pos - v) < 2.5 then
                            DrawText3D(v.x, v.y, v.z, "Personal Förråd")
                        end
                    end
                end
            end
        ...
        if LocalPlayer.state.isLoggedIn and PlayerJob.name == "police" then
            ...
            for k, v in pairs(Config.Locations["trash"]) do
                if #(pos - v) < 2 then
                    sleep = 5
                    if #(pos - v) < 1.0 then
                        DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Papperskorg")
                        if IsControlJustReleased(0, 38) then
                    ...
                    elseif #(pos - v) < 1.5 then
                        DrawText3D(v.x, v.y, v.z, "Papperskorg")
                    end
                end
            end
        end
            ...
            for k, v in pairs(Config.Locations["fingerprint"]) do
                if #(pos - v) < 4.5 then
                    if onDuty then
                        sleep = 5
                        if #(pos - v) < 1.5 then
                            DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Skanna fingeravtryck")
                            if IsControlJustReleased(0, 38) then
                                local player, distance = GetClosestPlayer()
                        ...
                        elseif #(pos - v) < 2.5 then
                            DrawText3D(v.x, v.y, v.z, "Fingerskanning")
                        end
                    end
                end
            end
            ...
            for k, v in pairs(Config.Locations["armory"]) do
                if #(pos - v) < 4.5 then
                    if onDuty then
                        sleep = 5
                        if #(pos - v) < 1.5 then
                            DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Vapenskåp")
                            if IsControlJustReleased(0, 38) then
                        ...
                        elseif #(pos - v) < 2.5 then
                            DrawText3D(v.x, v.y, v.z, "Vapenskåp")
                        end
                    end
                end
            end
                    ...
                    if onDuty then
                        ...
                        if #(pos - vector3(v.x, v.y, v.z)) < 1.5 then
                            if IsPedInAnyVehicle(PlayerPedId(), false) then
                                DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Parkera Helikopter")
                            else
                                DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Ta ut Helikopter")
                            end
                            if IsControlJustReleased(0, 38) then
                                if IsPedInAnyVehicle(PlayerPedId(), false) then
            ...
            for k, v in pairs(Config.Locations["impound"]) do
                if #(pos - vector3(v.x, v.y, v.z)) < 7.5 then
                    if onDuty then
                        ...
                        if #(pos - vector3(v.x, v.y, v.z)) <= 1.5 then
                            if IsPedInAnyVehicle(PlayerPedId(), false) then
                                DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Beslagtagna Fordon")
                            else
                                if not headerDrawn then
                                    headerDrawn = true
                                    exports['qb-menu']:showHeader({
            ...
            for k, v in pairs(Config.Locations["vehicle"]) do
                if #(pos - vector3(v.x, v.y, v.z)) < 7.5 then
                    if onDuty then
                        ...
                        if #(pos - vector3(v.x, v.y, v.z)) < 1.5 then
                            if IsPedInAnyVehicle(PlayerPedId(), false) then
                                DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Förvara Fordon")
                            else
                                if not headerDrawn then
                                    headerDrawn = true
                                    exports['qb-menu']:showHeader({
