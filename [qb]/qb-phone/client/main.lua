...
local function OpenPhone()
    QBCore.Functions.TriggerCallback('qb-phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            ...
            exports['okokNotify']:Alert("Telefon", "Du har ingen mobil", 3500, 'error')
        end
    end)
end
...
local function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Telefon",
                text = "Du har inget inkommande samtal...",
                icon = "fas fa-phone",
                color = "#e84118",
            },
        })
...
RegisterCommand('phone', function()
    ...
    if not PhoneData.isOpen and LocalPlayer.state.isLoggedIn then
        if not PlayerData.metadata['ishandcuffed'] and not PlayerData.metadata['inlaststand'] and not PlayerData.metadata['isdead'] and not IsPauseMenuActive() then
            ...
            exports['okokNotify']:Alert("Telefon", "Åtgärden är inte tillgänglig just nu..", 3500, 'error')
        end
    end
end)
...
if findVehFromPlateAndLocate(veh.plate) then
    exports['okokNotify']:Alert("Telefon", "Ditt fordon har märkts", 3500, 'success')
else
    exports['okokNotify']:Alert("Telefon", "Detta fordon kan inte hittas", 3500, 'error')
end
end)
...
RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    QBCore.Functions.TriggerCallback('qb-lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            ...
            QBCore.Functions.Notify('Du är för långt borta från loppet. GPS-uppsättning.', 'error', 5000)
...
RegisterNUICallback('CanTransferMoney', function(data, cb)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
		...
		SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { timeout=3000, title = "Bank", text = "Kontot finns inte!", icon = "fas fa-university", color = "#ff0000", }, })
    ...
    while takePhoto do
        ...
        elseif IsControlJustPressed(1, 176) then
            QBCore.Functions.TriggerCallback("qb-phone:server:GetWebhook",function(hook)
                QBCore.Functions.Notify('Rättar till foto...', 'primary')
                exports['screenshot-basic']:requestScreenshotUpload(tostring(hook), "files[]", function(data)
                    ...
                    exports['okokNotify']:Alert("Telefon", "Foto Sparad!", 3500, 'success')
                    OpenPhone()
                end)
            end)
...
RegisterNetEvent('qb-phone:client:NewMailNotify', function(MailData)
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Gmail",
            text = "Nytt e-postmeddelande från: "..MailData.sender,
...
RegisterNetEvent('qb-phone:client:BillingEmail', function(data, paid, name)
    if paid then
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = 'Faktureringsavdelning',
            subject = 'betald faktura',
            message = 'Fakturan har betalats från '..name..' I ett belopp av SEK'..data.amount,
        })
    else
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = 'Faktureringsavdelning',
            subject = 'Faktura avvisad',
            message = 'Fakturan har avvisats '..name..' I ett belopp av SEK'..data.amount,
        })
    end
end)
...
RegisterNUICallback('phone-silent-button', function(data,cb)
    if CallVolume == tonumber("0.2") then
        CallVolume = 0
        exports['okokNotify']:Alert("Telefon", "Tyst läge på", 3500, 'success')
        cb(true)
    else
        CallVolume = 0.2
        exports['okokNotify']:Alert("Telefon", "Tyst läge av", 3500, 'error')
        cb(false)
    end
end)
        ...
        if PhoneData.isOpen then
            if SenderNumber ~= PhoneData.PlayerData.charinfo.phone then
                SendNUIMessage({
                    ...
                    PhoneNotify = {
                        title = "Meddelande",
                        text = "Nytt meddelande: "..IsNumberInContacts(SenderNumber),
                        icon = "fas fa-comment",
                        color = "#25D366",
                        timeout = 1500,
                    },
                ...
                SendNUIMessage({
                    ...
                    PhoneNotify = {
                        title = "Meddelande",
                        text = "Har du skickat ett meddelande till dig själv?",
                        icon = "fas fa-comment",
                        color = "#25D366",
                        timeout = 4000,
                    },
...
RegisterNetEvent('qb-phone:client:RemoveBankMoney', function(amount)
    if amount > 0 then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Bank",
                text = "SEK"..amount.." bort från ditt saldo!",
                icon = "fas fa-university",
                color = "#ff002f",
                timeout = 3500,
            },
...
RegisterNetEvent('qb-phone:client:GiveContactDetails', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        exports['okokNotify']:Alert("Telefon", "Ingen i närheten", 3500, 'error')
    end
end)
