...
function timeAgo(dateParam) {
  ...
  if (seconds < 5) {
    ...
    return `${seconds} Någora sekunder sen`;
  } else if (seconds < 90) {
...
$(document).ready(() => {
    ...
    if (canSaveProfile == true) {
      ...
      setTimeout(() => {
        ...
        $(".tags-holder")
          .find("div")
          .each(function () {
            if ($(this).text() != "" && $(this).text() != "Inga Taggar") {
              tags.push($(this).text());
            }
        });
      ...
      if (canSaveProfile == true) {
            ...
            .each(function (index) {
              if ($(this).data("id") == cid) {
                if ($(this).hasClass("green-tag")) {
                  ...
                  if ($(this).text() == "Motivera") {
                    warrant = true;
                  }
                  if ($(this).text() == "Bearbetat") {
                    processed = true;
                  }
        ...
        setTimeout(() => {
          if (canRefreshIncidents == true) {
            ...
            setTimeout(() => {
              $(".incidents-search-refresh").empty();
              $(".incidents-search-refresh").html("Ladda");
  ...
  $(".manage-incidents-title-holder").on(
    ...
    function () {
      let tempalte =
        "📝 Sammanfattning:\n\n[Infoga rapportsammanfattning här]\n\n🧍 Gisslan: [Namn här]\n\n🔪 Vapen/föremål konfiskerade:\n\n· [Infoga lista här]\n\n-----\n💸 Böter:\n⌚ Mening:\n-----";
      $("#manage-incidents-title-input").val(
        "Namn - Avgift - " + $(".date").html()
      );
      ...
      $(".manage-incidents-editing-title").html(
        "Du skapar för närvarande en ny Incident"
      );
  ...
  $(".manage-bolos-title-holder").on(
    ...
    function () {
      ...
      if ($(".badge-logo").attr("src") == "img/ems_badge.png") {
        template =
          "ICU-rum #: [ # ]\n\nRapport ID: [ Rapport ID ]\n\nTid antagen: [ Datum och tid här ]\n\nKirurgi: [Ja/Nej]\n\nSkador/åkommor:\n - [ Ange listan över skador här ]\n\n\nYtterligare deltagande:\n - [ Lista annan personer här ]\n\n\n🧑‍🤝‍🧑 Ytterligare nödkontakter:\n - [ Namn och nummer ]\n\n\nAnteckningar:\n[Ytterligare anteckningar här]";
      }
      $(".manage-bolos-editing-title").html(
        "Du skapar för närvarande en ny BOLO"
      );
  ...
  $(".bolos-search-refresh").click(function () {
    if (canRefreshBolo == true) {
      ...
      setTimeout(() => {
        $(".bolos-search-refresh").empty();
        $(".bolos-search-refresh").html("Ladda");
        canRefreshBolo = true;
    ...
    function () {
      ...
      $(".incidents-ghost-holder").prepend(
        `
          ...
          this
        ).data("cid")}">Motivera</div>
                    <div class="associated-incidents-user-tag red-tag" data-id="${$(
          this
        ).data("cid")}">Skyldig</div>
                    <div class="associated-incidents-user-tag red-tag" data-id="${$(
          this
        ).data("cid")}">Bearbetat</div>
                    <div class="associated-incidents-user-tag red-tag" data-id="${$(
          this
  ...
  $(".reports-search-refresh").click(function () {
    if (canRefreshReports == true) {
      ...
      setTimeout(() => {
        $(".reports-search-refresh").empty();
        $(".reports-search-refresh").html("Ladda");
  ...
  $(".dispatch-comms-refresh").click(function () {
    ...
    setTimeout(() => {
      $(".dispatch-comms-refresh").empty();
      $(".dispatch-comms-refresh").html("Ladda");
      canRefreshReports = true;
  ...
  $(".manage-reports-title-holder").on(
    ...
    function () {
      ...
      if ($(".badge-logo").attr("src") == "img/ems_badge.png") {
        template =
          "Inlämnad till ICU?: [Yes/No]\n\nIncidentrapport:\n[ Kort sammanfattning av vad som hände och vem som gjorde vad på plats. Notera allt som stack ut om scenen samt vad som gjordes för att behandla patienten ]\n\n\nLista över skador:\n- [ Ange vilken eller vilka skador som inträffade ]\n\n\n💉 Kirurgisk rapport:\ n[ Fullständig rapport om vad som gjordes under operationen, ange eventuella komplikationer eller något som hittades under operationen. Notera vem som deltog och vad de gjorde under operationen. I slutet av rapporten var noga med att notera patientens tillstånd efter ]\n\n\nBevakning:\n- [ Lista alla som deltar här ]\n\n\nTillämpade mediciner:\n- [ Lista alla närvarande här ] \n\n\nAnteckningar:\n[ Ytterligare anteckningar här ]";
      }
                            ...
                            <div style="display: flex; flex-direction: column;">
                                <div class="dmv-item-title">${value.model}</div>
                                    <div class="dmv-tags">
                                        ...
                                        <div class="dmv-tag ${impound}">Beslagtagen</div>
                                        <div class="dmv-tag ${bolo}">BOLO</div>
                                        <div class="dmv-tag ${stolen}">Stulen</div>
                                        <div class="dmv-tag ${codefive}">Kod 5</div>
                                    </div>
                                </div>
                                <div class="dmv-bottom-info">
                                    <div class="dmv-id">Plate: ${value.plate} · Ägare: ${value.owner}</div>
                                </div>
                            </div>
                        </div>
                    `;
  ...
  function JobColors(sentJob) {
    if (sentJob) {
      if (PoliceJobs[sentJob] !== undefined) {
        ...
        $(".header-title").html("DRÖMSTADENS POLISMYDIGHET");
        $(".bolo-nav-item").html("BOLOs");
        $(".bolos-search-title").html("Bolos");
        $("#bolos-search-input").attr("placeholder", "Sök Bolo...");
        ...
        $("#boloplate").attr("placeholder", "Placera reggplåt här...");
        $("#bolodetail").attr(
          "placeholder",
          "Bolo detalj kommer här..."
        );
        $("#boloowner").attr(
          "placeholder",
          "Placera fordonsägaren här..."
        );
        $("#boloindividual").attr(
          "placeholder",
          "Placera invidivual här..."
        );
  ...
  window.addEventListener("message", function (event) {
    ...
    if (eventData.type == "show") {
      ...
      activeUnits.forEach((unit) => {
        ...
        let activeInfoJob = `<div class="unit-job active-info-job-unk">OKÄND</div>`;
        if (PoliceJobs[playerJob] !== undefined) {
          ...
          activeInfoJob = `<div class="unit-job active-info-job-ambulance">LÄKARE</div>`
          emsCount++;
