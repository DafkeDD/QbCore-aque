...
local function RegisterStashTarget()
    ...
    if Config.UseTarget then
        ...
        zone:onPlayerInOut(function (isPointInside)
            if isPointInside then
                exports['qb-core']:DrawText("[E] Öppna Förråd", 'left')
            else
                exports['qb-core']:HideText()
            end
...
local function RegisterGarageZone()
    ...
    vehicleZone:onPlayerInOut(function (isPointInside)
        if isPointInside and onDuty then
            ...
            if inVehicle then
                exports['qb-core']:DrawText('[E] Göm Fordon', 'left')
            else
                exports['qb-core']:DrawText('[E] Placera Fordon', 'left')
            end
        else
            exports['qb-core']:HideText()
        end
    ...
    plateZone:onPlayerInOut(function (isPointInside)
        if isPointInside and onDuty then
            if plate.AttachedVehicle then
                exports['qb-core']:DrawText('[E] Öppna Meny', 'left')
            else
                if IsPedInAnyVehicle(PlayerPedId()) then
                    exports['qb-core']:DrawText('[E] Jobba På Fordon', 'left')
                end
            end
        else
            exports['qb-core']:HideText()
...
local function SetVehiclePlateZones()
    if Config.Plates and next(Config.Plates) then
        ...
        print('Inga fordonsskyltar konfigurerade')
    end
end
...
local function OpenMenu()
    local OpenMenu = {
        {
            header = "Fordonsalternativ",
            isMenuHeader = true
        }, {
            header = "Koppla bort fordonet",
            txt = "Lossa fordon i hissen",
            params = {
                event = "qb-mechanicjob:client:UnattachVehicle",
            }
        }, {
            header = "Kolla statusen",
            txt = "Kontrollera fordonsstatus",
            params = {
...
local function PartMenu(data)
    ...
    local TestMenu1 = {
        ...
        {
            header = ""..partName.."",
            txt = "Laga : "..QBCore.Shared.Items[Config.RepairCostAmount[part].item]["label"].." "..Config.RepairCostAmount[part].costs.."x", 
            params = {
...
local function NoDamage()
    local NoDamage = {
        ...
        {
            header = "Tillbaka",
            txt = "Det finns ingen skada på denna del!",
            params = {
                event = "qb-mechanicjob:client:PartsMenu",
            }
        },
    ...
    QBCore.Functions.TriggerCallback('qb-inventory:server:GetStashItems', function(StashItems)
        ...
        if hasitem and countitem >= PartData.costs then
            ...
            QBCore.Functions.Progressbar("repair_part", "Repairing " ..Config.ValuesLabels[part], math.random(5000, 10000), false, true, {
                ...
                QBCore.Functions.Notify("Reparation avbruten", "error")
            end)
        else
            QBCore.Functions.Notify('Det finns inte tillräckligt med material i kassaskåpet', 'error')
        end
    end, "mechanicstash")
end
...
RegisterNetEvent('qb-vehicletuning:client:RepaireeePart', function(part)
    ...
    QBCore.Functions.Notify(""..Config.ValuesLabels[part].." är reparerad!")
end)
            ...
            if #(pos - vehpos) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil then
                        ...
                        QBCore.Functions.Notify("Status okänd", "error")
                    end
                else
                    QBCore.Functions.Notify("Inte ett giltigt fordon", "error")
                end
            else
                QBCore.Functions.Notify("Du är inte tillräckligt nära fordonet", "error")
            end
        else
            QBCore.Functions.Notify("Du måste vara i fordonet först", "error")
        end
    else
        QBCore.Functions.Notify("Du måste vara utanför fordonet", "error")
    end
end)
...
RegisterNetEvent('vehiclemod:client:fixEverything', function()
    if (IsPedInAnyVehicle(PlayerPedId(), false)) then
        ...
        if not IsThisModelABicycle(GetEntityModel(veh)) and GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
            ...
            QBCore.Functions.Notify("Du är inte föraren eller på en cykel", "error")
        end
    else
        QBCore.Functions.Notify("Du sitter inte i ett fordon", "error")
    end
end)
            ...
            if #(pos - vehpos) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                        ...
                        QBCore.Functions.Progressbar("repair_advanced", "Laga Fordon", lockpickTime, false, true, {
                            disableMovement = true,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true,
                            ...
                            openingDoor = false
                            ClearPedTasks(PlayerPedId())
                            QBCore.Functions.Notify("Processen avbruten", "error")
                        end)
                    else
                        QBCore.Functions.Notify("Inte en giltig del", "error")
                    end
                else
                    QBCore.Functions.Notify("Inte ett giltigt fordon", "error")
                end
...
CreateThread(function ()
    ...
    AddTextComponentSubstringPlayerName("Redline")
