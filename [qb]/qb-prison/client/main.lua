...
RegisterNetEvent('prison:client:Enter', function(time)
	...
	TriggerEvent("chatMessage", "SYSTEM", "warning", "Din egendom har tagits i beslag, du får tillbaka allt när din tid är ute..")
	DoScreenFadeOut(500)
...
RegisterNetEvent('prison:client:Leave', function()
	if jailTime > 0 then
		...
		TriggerEvent("chatMessage", "SYSTEM", "warning", "du har fått tillbaka din egendom..")
		inJail = false
		RemoveBlip(currentBlip)
		RemoveBlip(CellsBlip)
		CellsBlip = nil
...
freedom:onPlayerInOut(function(isPointInside)
	if isPointInside then
		exports['qb-core']:DrawText('[E] Kolla Iden', 'left')
		insidefreedom = true
	else
		insidefreedom = false
...
canteen:onPlayerInOut(function(isPointInside)
	if isPointInside then
		exports['qb-core']:DrawText('[E] Öppna Butiker', 'left')
		insidecanteen = true
	else
		insidecanteen = false
		exports['qb-core']:HideText()
