...
--Fuel siphon event
RegisterNetEvent("LegacyFuel:client:siphonfuel",function() 
    ...
    if distanceToVehicle > 2.5 then
        QBCore.Functions.Notify("Du är för långt bort från fordonet","error")
        return
    end

    --Check petrol can is able to take fuel
    if petrolCanDurability == nil then
        QBCore.Functions.Notify("Du behöver en bensindunk i dina händer","error")
        return
    elseif petrolCanDurability == 100 then
        QBCore.Functions.Notify("Din bensindunk är full","error")
        return
    end
    ...
    if currentFuel > 0 then
        ...
        QBCore.Functions.Notify("Du sugde bränsle","success")
    else
        QBCore.Functions.Notify("Tanken är tom","error")
    end
...
--Action events
RegisterNetEvent("LegacyFuel:client:refillpetrolcan", function()
    local petrolCanDurability = GetCurrentGasCanDurability()
    if petrolCanDurability ~= nil then
        if petrolCanDurability == 100 then
            QBCore.Functions.Notify("Din dunk är full","error")
        else
            local refillCost = math.floor(100 - petrolCanDurability)
            if refillCost > 0 then
                local currentCash = QBCore.Functions.GetPlayerData().money['cash']  
			    if currentCash >= refillCost then
					TriggerServerEvent('LegacyFuel:server:pay', refillCost, GetPlayerServerId(PlayerId()))
					SetPetrolCanDurability(100)
				    QBCore.Functions.Notify("Du fyllde på din bensindunk","success")
                else
                    QBCore.Functions.Notify("Inte tillräckligt med kontanter för att fylla på burken","error")
                end
            end
        end
    else
        QBCore.Functions.Notify("Du har ingen bensindunk att fylla på","error")
    end
end)

RegisterNetEvent("LegacyFuel:client:buypetrolcan", function()
    local currentCash = QBCore.Functions.GetPlayerData().money['cash']
    if currentCash >= Config.JerryCanCost then
		...
		TriggerServerEvent('LegacyFuel:server:pay', Config.JerryCanCost, GetPlayerServerId(PlayerId()))
		QBCore.Functions.Notify("Du köpte en jerry can","success")
	else
		QBCore.Functions.Notify("Du har inte tillräckligt med pengar för att köpa en jerryburk","error")
	end
end)

RegisterNetEvent("LegacyFuel:client:pumprefuel", function(pump) 
    local PlayerPed = PlayerPedId()
    local Vehicle = QBCore.Functions.GetClosestVehicle()
    ...
    if CanFuelVehicle(Vehicle) then
        ...
        while isFueling do
            ...
            local extraString = "\n" .. "Kostnad " .. ": ~b~SEK" .. Round(currentCost, 1)
		...
		TriggerServerEvent('LegacyFuel:server:pay', currentCost, GetPlayerServerId(PlayerId()))
        QBCore.Functions.Notify("Du betalde" .. currentCost .. " SEK för tanken","success")
    else
        QBCore.Functions.Notify("Din tank är full","error")
    end

end)

RegisterNetEvent("LegacyFuel:client:petrolcanrefuel", function() 
    local PlayerPed = PlayerPedId()
    local Vehicle = QBCore.Functions.GetClosestVehicle()
    ...
    elseif petrolCanDurability <= 0 then
        QBCore.Functions.Notify("Din bensindunk är tom","error")
        return
    end
    ...
    if CanFuelVehicle(Vehicle) then
        ...
        ClearPedTasks(PlayerPed)
        QBCore.Functions.Notify("Du tankar din bil","success")
    else
        QBCore.Functions.Notify("Tanken är full","error")
    end
