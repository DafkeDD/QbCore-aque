        ...
        if LocalPlayer.state.isLoggedIn and not keyPressed and GetLastInputMethod(0) then
            ...
            if next(keyMeta) ~= nil then
                if keyMeta[v[2]]["command"] ~= "" then
                    ...
                    QBCore.Functions.Notify('Det finns fortfarande inget ['..v[2]..'] bundet, /binds för att binda ett kommando', 'primary', 4000)
                end
            else
                QBCore.Functions.Notify('Du har inte bundit några kommandon, /binds för att binda ett kommando', 'primary', 4000)
            end
        end
    end, false)
