...
RegisterNetEvent('doj:client:useBurnerPhone', function()
	...
	if cooldown then
		QBCore.Functions.Notify('Du kan bara sälja en gång var 5:e minut.', 'error')
		return
	end
	...
	if (Area == Config.cornerSellLocation.area1) or (Area == Config.cornerSellLocation.area2) or (Area == Config.cornerSellLocation.area3) or (Area == Config.cornerSellLocation.area4) or (Area == Config.cornerSellLocation.area5) or (Area == Config.cornerSellLocation.area6) or (Area == Config.cornerSellLocation.area7) or (Area == Config.cornerSellLocation.area8) or (Area == Config.cornerSellLocation.area9) or (Area == Config.cornerSellLocation.area10) then
		sellingweed = true
		cooldown = true
		QBCore.Functions.Notify('Det här ser ut som ett bra område', 'success', 5000)
		...
		Wait(7000)
		ClearPedTasks(ped)
		QBCore.Functions.Notify('Säljer aktivt vid '..Area, 'success')
	else
		QBCore.Functions.Notify('Det är ingen som letar efter det här', 'error')
		return
	end
    ...
    while sellingweed do
		...
		if sellingweed then
			...
			if distance > 75.0 then
				QBCore.Functions.Notify('Du flyttade för långt från ditt område!', 'error')
				EndSelling()
			end
			local RandomNPC = GetRandomNPC()
			if RandomNPC then
				QBCore.Functions.Notify('Någon närmar sig', 'primary')
			end
...
RegisterNetEvent('doj:client:AllowSale', function(NPC)
	...
	while true do
		...
		if distance < 2.0 then
			exports['textUi']:DrawTextUi('show', "[E] - Sälj [H] - Shoo")
			if IsControlJustReleased(2,38) and distance < 2.0 then
				local chance = math.random(1, 20) 
				if chance <= 5 then
					QBCore.Functions.Notify('Köparen har ändrat sig.', 'error')
					TaskWanderStandard(NPC, 10.0, 10)
					...
					QBCore.Functions.TriggerCallback('doj:server:checkEights', function(result)
						if result ~= nil then
							...
							exports['textUi']:DrawTextUi('hide')
							EndSelling()
							QBCore.Functions.Notify('Du har inga åttor att sälja', 'error')
						end
					end)
					ClearPedTasks(NPC)
...
function EndSelling()
	QBCore.Functions.Notify('Hörnförsäljningen upphörde!', 'error')
	sellAmount = 0
	sellingweed = false
end 
...
function cornerSellingMenu()
	...
	exports['qb-menu']:openMenu({
		{
			header = "Hörn Säljer 3,5gs",
			txt = "",
			isMenuHeader = true
		},
		{
			header = "Låg grad",
			txt = "Erbjudande: SEK" .. Config.lowGradeEighthPrice,
			params = {
		...
		{
			header = "Mellanklass",
			txt = "Erbjudande: SEK" .. Config.midGradeEighthPrice,
			params = {
				event = "doj:server:sellEighth",
				isServer = true,
				args = 2
		...
		{
			header = "Höggradig",
			txt = "Erbjudande: SEK" .. Config.highGradeEighthPrice,
			params = {
				event = "doj:server:sellEighth",
				isServer = true,
				args = 3
