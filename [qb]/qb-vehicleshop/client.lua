...
local vehHeaderMenu = {
    {
        ...
        txt = 'Interagera med det aktuella fordonet',
        params = {
...
local financeMenu = {
    {
        ...
        txt = 'Bläddra bland dina ägda fordon',
        params = {
    ...
    CreateThread(function()
        while inTestDrive do
            if GetGameTimer() < gameTimer + tonumber(1000 * testDriveTime) then
                ...
                drawTxt('Testkörningstid som återstår: '..math.ceil(testDriveTime - secondsLeft / 1000), 4, 0.5, 0.93, 0.50, 255, 255, 255, 180)
            end
            Wait(0)
        end
    end)
            ...
            CreateThread(function()
                while insideZones[name] do
                    ...
                    vehicleMenu = {
                        ...
                        {
                            header = 'Provkör',
                            txt = 'Provkör det aktuella fordonet',
                            params = {
                        ...
                        {
                            header = 'Finansieringsfordon',
                            txt = 'Finans för närvarande valt fordon',
                            params = {
                        ...
                        {
                            header = 'Byt fordon',
                            txt = 'Ändra för närvarande valda fordon',
                            params = {
            ...
            CreateThread(function()
                while insideZones[name] and PlayerData.job and PlayerData.job.name == Config.Shops[name]['Job'] do
                    ...
                    vehicleMenu = {
                        ...
                        {
                            header = 'Provköra',
                            txt = 'Tillåt spelaren att provköra',
                            params = {
                        ...
                        {
                            header = 'Finansiella fordon',
                            txt = 'Finansieringsfordon till spelare',
                            params = {
...
RegisterNetEvent('qb-vehicleshop:client:TestDrive', function()
    if not inTestDrive and ClosestVehicle ~= 0 then
        ...
        QBCore.Functions.SpawnVehicle(Config.Shops[getShopInsideOf()]["ShowroomVehicles"][ClosestVehicle].chosenVehicle, function(veh)
            ...
            SetVehicleNumberPlateText(veh, 'PROVKÖR')
            SetEntityAsMissionEntity(veh, true, true)
            SetEntityHeading(veh, Config.Shops[closestShop]["VehicleSpawn"].w)
            TriggerEvent('vehiclekeys:client:SetOwner', QBCore.Functions.GetPlate(veh))
            TriggerServerEvent('qb-vehicletuning:server:SaveVehicleProps', QBCore.Functions.GetVehicleProperties(veh))
            testDriveVeh = veh
            QBCore.Functions.Notify('Du har '..Config.Shops[closestShop]["TestDriveTimeLimit"]..' minuter kvar')
            SetTimeout(Config.Shops[closestShop]["TestDriveTimeLimit"] * 60000, function()
                if testDriveVeh ~= 0 then
                    ...
                    QBCore.Functions.Notify('Testkörning av fordon klar')
                end
            end)
        ...
        QBCore.Functions.Notify('Redan på provkörning', 'error')
    end
end)
...
RegisterNetEvent('qb-vehicleshop:client:customTestDrive', function(data)
    if not inTestDrive then
        ...
        QBCore.Functions.SpawnVehicle(vehicle, function(veh)
            ...
            TriggerServerEvent('qb-vehicletuning:server:SaveVehicleProps', QBCore.Functions.GetVehicleProperties(veh))
            testDriveVeh = veh
            QBCore.Functions.Notify('Du har '..Config.Shops[shopInsideOf]["TestDriveTimeLimit"]..' minuter kvar')
...
RegisterNetEvent('qb-vehicleshop:client:TestDriveReturn', function()
    ...
    if veh == testDriveVeh then
        ...
        QBCore.Functions.Notify('Det här är inte ditt provkörningsfordon', 'error')
    end
end)
...
RegisterNetEvent('qb-vehicleshop:client:openFinance', function(data)
    local dialog = exports['qb-input']:ShowInput({
        header = getVehBrand():upper()..' '..data.buyVehicle:upper()..' -'..data.price..'KR',
        ...
        inputs = {
            {
                ...
                isRequired = true,
                name = 'downPayment',
                text = 'Förskottsbelopp - Min '..Config.MinimumDown..'%'
            },
            {
                type = 'number',
                isRequired = true,
                name = 'paymentAmount',
                text = 'Totala betalningar - Max '..Config.MaximumPayments
            }
        }
    })
    if dialog then
...
RegisterNetEvent('qb-vehicleshop:client:swapVehicle', function(data)
    ...
    if Config.Shops[shopName]["ShowroomVehicles"][data.ClosestVehicle].chosenVehicle ~= data.toVehicle then
        ...
        SetVehicleNumberPlateText(veh, 'KÖP MIG')
...
RegisterNetEvent('qb-vehicleshop:client:getVehicleFinance', function(data)
    local vehFinance = {
        ...
        {
            isMenuHeader = true,
            header = 'Totalt återstående saldo',
            txt = 'KR'..comma_value(data.balance)
        },
        {
            isMenuHeader = true,
            header = 'Totala återstående betalningar',
            txt = data.paymentsLeft
        },
        {
            isMenuHeader = true,
            header = 'Återkommande betalningsbelopp',
            txt = 'KR'..comma_value(data.paymentAmount)
        },
        {
            header = 'Gör En Betalning',
            params = {
