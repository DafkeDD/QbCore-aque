...
function OpenMenu()
    exports['qb-menu']:openMenu({
      ...
      {
        header = "Starta Teoritest",
        txt = "SEK"..Config.Amount['theoretical'].."",
        params = {
...
function OpenMenu2()
  exports['qb-menu']:openMenu({
    ...
    {
      header = "Starta Körtest",
      txt = "SEK"..Config.Amount['driving'].."",
      params = {
    ...
    {
      header = "Starta CDL körtest",
      txt = "SEK"..Config.Amount['cdl'].."",
      params = {
...
RegisterNetEvent('qb-dmv:startdriver', function()
        ...
        QBCore.Functions.SpawnVehicle(Config.VehicleModels.driver, function(veh)
            ...
            CurrentVehicle = veh
            QBCore.Functions.Notify('Du tar körprovet')
...
function StopTheoryTest(success) 
    ...
    SetNuiFocus(false)
    if success then
      QBCore.Functions.Notify('Du klarade ditt prov!', 'success')
      TriggerServerEvent('qb-dmv:theorypaymentpassed')
    else
      QBCore.Functions.Notify('Du misslyckades på testet!', 'error')
...
function StopDriveTest(success)
    ...
    if success then
      ...
      QBCore.Functions.Notify('Du klarade körprovet!')
      QBCore.Functions.DeleteVehicle(veh)
      CurrentTest = nil
    elseif success == false then
      TriggerServerEvent('qb-dmv:driverpaymentfailed')
      QBCore.Functions.Notify('Du misslyckades på körprovet!')
      CurrentTest = nil
      ...
      if CurrentTest == 'drive' then
          ...
          if IsPedInAnyVehicle(playerPed,  false) then
              ...
              for k,v in pairs(Config.SpeedLimits) do
                  ...
                  tooMuchSpeed = true
                      if not IsAboveSpeedLimit then
                          ...
                          QBCore.Functions.Notify('Kör för fort',"error")
              ...
              if health < LastVehicleHealth then
                  DriveErrors = DriveErrors + 1
                  QBCore.Functions.Notify('Du skadade fordonet')
                  QBCore.Functions.Notify("Errors:"..tostring(DriveErrors).."/" ..Config.MaxErrors, "error")
...
Citizen.CreateThread(function ()
    while true do
        ...
        if dist <= 6.0 then
            ...
            DrawText3Ds(marker['x'], marker['y'], marker['z'], "[E] Körskola")
            if dist <= 1.5 then
              if CurrentTest ~= 'drive' then
                      ...
                      if permit == false then
                          QBCore.Functions.TriggerCallback('qb-dmv:server:menu2', function (license)
                              if license then
                                ...
                                QBCore.Functions.Notify("Du har redan tagit dina test! Gå till Stadshuset för att köpa din licens.")
                              end
                          end)
                      else
                        Wait(10)
              ...
              elseif CurrentTest == 'drive' and IsControlJustReleased(0, 46) then
                QBCore.Functions.Notify("Du tar redan förarprovet.")
              end
            end
        end
    end
...
CreateThread(function()
  local blip = AddBlipForCoord(177.28, -1319.92, 29.5)
  ...
  BeginTextCommandSetBlipName("STRING")
  AddTextComponentString('Pawn Butiken')
  EndTextCommandSetBlipName(blip)
end)