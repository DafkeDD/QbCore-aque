...
function openLocker(bankId, lockerId) -- Globally Used
    ...
    if bankId == "paleto" then
        QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
            if hasItem then
                ...
                IsDrilling = true
                QBCore.Functions.Progressbar("open_locker_drill", "Bryter upp kassaskåpet..", math.random(18000, 30000), false, true, {
                    ...
                    QBCore.Functions.Notify("Lyckades!", "success")
                    IsDrilling = false
                end, function() -- Cancel
                    StopAnimTask(ped, "anim@heists@fleeca_bank@drilling", "drill_straight_idle", 1.0)
                    TriggerServerEvent('qb-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                    DetachEntity(DrillObject, true, true)
                    DeleteObject(DrillObject)
                    QBCore.Functions.Notify("Avbryten..", "error")
                    IsDrilling = false
                end)
                CreateThread(function()
                    while IsDrilling do
                ...
                QBCore.Functions.Notify("Det verkar som att kassalåset är för starkt..", "error")
                TriggerServerEvent('qb-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
            end
        end, "drill")
    elseif bankId == "pacific" then
        ...
        IsDrilling = true
        QBCore.Functions.Progressbar("open_locker", "Bryter upp kassaskåpet..", math.random(27000, 37000), false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
                ...
                if not Config.SmallBanks[closestBank]["isOpened"] then
                    QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                        if result then
                            ...
                            QBCore.Functions.Progressbar("hack_gate", "Ansluter hackningsenheten ..", math.random(5000, 10000), false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                            ...
                            QBCore.Functions.Notify("Du saknar ett föremål...", "error")
                        end
                    end, {["trojan_usb"] = 1, ["electronickit"] = 1})
                else
                    QBCore.Functions.Notify("Ser ut som att banken redan är öppen..", "error")
                end
            else
                QBCore.Functions.Notify('Minst av '..Config.MinimumFleecaPolice..' Polis behövs', "error")
            end
        else
            QBCore.Functions.Notify("Säkerhetslåset är aktivt, öppning av dörren är för närvarande inte möjligt.", "error", 5500)
        end
    end)
end)
    ...
    if PlayerJob.name == "police" and PlayerJob.onduty then
        ...
        if type == "small" then
            ...
            TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
                timeOut = 10000,
                alertTitle = "Fleeca bankrån försök",
                coords = {
                    x = coords.x,
                    y = coords.y,
                    z = coords.z,
        ...
        elseif type == "paleto" then
            ...
            TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
                timeOut = 10000,
                alertTitle = "Blain County Savings bankrån försök",
                coords = {
                    x = coords.x,
                    y = coords.y,
                    z = coords.z,
        ...
        elseif type == "pacific" then
            ...
            TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
                timeOut = 10000,
                alertTitle = "Pacific Standard Bank rånförsök",
                coords = {
                    x = coords.x,
                    y = coords.y,
                    z = coords.z,
        ...
        AddTextComponentString("10-90: Bankrån")
            ...
            if Config.UseTarget then
                ...
                lockerZone:onPlayerInOut(function(inside)
                    if inside and closestBank ~= 0 and Config.SmallBanks[i]["isOpened"] and not Config.SmallBanks[i]["lockers"][k]["isOpened"] and not Config.SmallBanks[i]["lockers"][k]["isBusy"] then
                        exports['qb-core']:DrawText('[E] Bryt upp kassaskåpet', 'right')
                        currentLocker = k
                    else
                ...
                if currentLocker ~= 0 then
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        if CurrentCops >= Config.MinimumFleecaPolice then
                            ...
                            QBCore.Functions.Notify('Minsta av '..Config.MinimumFleecaPolice..' Polisen Behövs', "error")
                        end
                        sleep = 1000
                    end
                end
