...
RegisterNetEvent('police:client:spawnCone', function()
    QBCore.Functions.Progressbar("spawn_object", "Placera objekt..", 25, false, true, {
        ...
        StopAnimTask(PlayerPedId(), "anim@narcotics@trash", "drop_front", 1.0)
        QBCore.Functions.Notify("Avbrytit..", "error")
    end)
end)

RegisterNetEvent('police:client:spawnmedic', function()
    QBCore.Functions.Progressbar("spawn_object", "Placera objekt..", 25, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        ...
        StopAnimTask(PlayerPedId(), "anim@narcotics@trash", "drop_front", 1.0)
        TriggerServerEvent("police:server:spawnObject", "medic")
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "anim@narcotics@trash", "drop_front", 1.0)
        QBCore.Functions.Notify("Avbrytit..", "error")
    end)
...
RegisterNetEvent('police:client:deleteObject', function()
    ...
    if dist < 5.0 then
        QBCore.Functions.Progressbar("remove_object", "Tar bort föremål..", 20, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
...
RegisterNetEvent('police:client:SpawnSpikeStrip', function()
    if #SpawnedSpikes + 1 < SpikeConfig.MaxSpikes then
        ...
        QBCore.Functions.Notify('Det finns inga Spikmattor kvar..', 'error')
    end
end)
            ...
            if ClosestSpike then
                ...
                if dist < 4 then
                    if not IsPedInAnyVehicle(PlayerPedId()) then
                        if PlayerJob.name == "police" and PlayerJob.onduty then
                            DrawText3D(pos.x, pos.y, pos.z, '[E] Ta bort Spikmattor')
