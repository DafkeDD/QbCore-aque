					...
					row = `	<div class="col col-md-4">
								<div class="card h-100">
								  <div class="card-body text-center mainmenu-subcard" id="menuMyInvoices" style="background-color: #eeeff3; color: #2f3037;">
								    ...
								    <p class="card-text">Personliga Fakturor</p>
								  </div>
								</div>
							</div>
							<div class="col col-md-4">
								<div class="card h-100">
								  <div class="card-body text-center mainmenu-subcard" id="menuSocietyInvoices" style="background-color: #eeeff3; color: #2f3037;">
								    ...
								    <p class="card-text">Företags Fakturor</p>
								  </div>
								</div>
							</div>
							<div class="col col-md-4">
								<div class="card h-100">
								  <div class="card-body text-center mainmenu-subcard" id="menuCreateInvoice" style="background-color: #eeeff3; color: #2f3037;">
								    ...
								    <p class="card-text">Skapa Faktura</p>
								  </div>
								</div>
							</div>
						  `
			...
			if (!windowIsOpened) {
				...
				for(var i=0; i<event.data.invoices.length; i++)
				{
					...
					if (invoices.status == 'paid') {
						tablestatus = '<td class="text-center align-middle"><span class="badge bg-success" style="font-size: 14px;"><i class="fas fa-check-circle"></i> BETALD</span></td>';
						modalstatus = '<span class="badge bg-success" style="position: absolute; right: 5%; top: 5%; font-size: 18px;"><i class="fas fa-check-circle"></i> BETALD</span>';
						payment_status = `<span style="font-size: 20px; color: #2f3037; text-transform: uppercase; font-weight: 600;">BETALD PÅ: ${invoices.paid_date}</span>`;
					}
					else if (invoices.status == 'unpaid') {
						tablestatus = '<td class="text-center align-middle"><span class="badge bg-danger" style="font-size: 14px;"><i class="fas fa-times-circle"></i> OBETALD</span></td>';
						modalstatus = '<span class="badge bg-danger" style="position: absolute; right: 5%; top: 5%; font-size: 18px;"><i class="fas fa-times-circle"></i> OBETALD</span>';
						payment_status = `<button type="button" id="" class="btn btn-blue flex-grow-1 payInvoice" style="border-radius: 10px; flex-basis: 100%;" data-invoiceId="${invoices.id}" data-invoiceMoney="${invoices.invoice_value}" data-bs-dismiss="modal"><i class="fas fa-shopping-bag"></i> BETALA ${invoices.invoice_value.toLocaleString()}&euro;</button>`;
					}
						...
						payment_status = `<span style="font-size: 20px; color: #2f3037; text-transform: uppercase; font-weight: 600;">AUTOGIRO PÅ: ${invoices.paid_date}</span>`;
					}
					else if (invoices.status == 'cancelled') {
						tablestatus = '<td class="text-center align-middle"><span class="badge bg-secondary" style="font-size: 14px;"><i class="fas fa-minus-circle"></i> AVBRYTEN</span></td>';
						modalstatus = '<span class="badge bg-secondary" style="position: absolute; right: 5%; top: 5%; font-size: 18px;"><i class="fas fa-minus-circle"></i> AVBRYTEN</span>';
						payment_status = '<span style="font-size: 20px; color: #2f3037; text-transform: uppercase; font-weight: 600;">AVBRYTEN</span>'
					}

					if (invoices.limit_pay_date == 'N/A') {
						limit_pay_date = ''
					} else {
						limit_pay_date = `<h6>Obetald: ${invoices.limit_pay_date}</h6>`
					}
					...
					row += `
						<tr>
							...
							<td class="text-center align-middle">${invoices.invoice_value.toLocaleString()}KR</td>
							<td class="text-center align-middle"><button type="button" class="btn btn-blue showInvoice" style="border-radius: 10px; flex-basis: 100%;" data-toggle="modal" data-target="#showInvoiceModal${invoices.id}" data-invoiceId="${invoices.id}""><i class="fas fa-eye"></i> VISA</button></td>
						</tr>
					`;
		...
		case 'societyinvoices':
			if (!windowIsOpened) {
				...
				for(var i=0; i<event.data.invoices.length; i++)
				{
					...
					$("#totalIncome").html(`${data.totalIncome.toLocaleString()}KR`);
					$("#unpaidInvoices").html(data.totalUnpaid);
					$("#awaitedIncome").html(`${data.awaitedIncome.toLocaleString()}KR`);
					...
					if (invoices.status == 'paid') {
						...
						tablestatus = '<td class="text-center align-middle"><span class="badge bg-danger" style="font-size: 14px;"><i class="fas fa-times-circle"></i> OBETALD</span></td>';
						modalstatus = '<span class="badge bg-danger" style="position: absolute; right: 5%; top: 5%; font-size: 18px;"><i class="fas fa-times-circle"></i> OBETALD</span>';
						payment_status = `<button type="button" id="" class="btn btn-red flex-grow-1 cancelInvoice" style="border-radius: 10px; flex-basis: 100%;" data-invoiceId="${invoices.id}" data-invoiceMoney="${invoices.invoice_value}" data-bs-dismiss="modal"><i class="fas fa-times-circle"></i> AVBRYT FAKTURA</button>`;
					}
					else if (invoices.status == 'autopaid') {
						tablestatus = '<td class="text-center align-middle"><span class="badge bg-info" style="font-size: 14px;"><i class="fas fa-clock"></i> AUTOGIRO</span></td>';
						modalstatus = '<span class="badge bg-info" style="position: absolute; right: 5%; top: 5%; font-size: 18px;"><i class="fas fa-clock"></i> AUTOGIRO</span>';
...
$(document).on('click', ".payInvoice", function() {
	...
	$.post('https://qb-faktura/action', JSON.stringify ({
		action: "payInvoice",
...
$(document).on('click', '#menuMyInvoices', function() {
	...
	$.post('http://qb-faktura/action', JSON.stringify ({
