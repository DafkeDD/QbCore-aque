	...
	QBCore.Functions.TriggerCallback('QBCore:HasItem', function(HasItem)
		if HasItem then
			if nearbyObject and IsPedOnFoot(playerPed) then
				isPickingUp = true
				QBCore.Functions.Notify('Skördar skörd...', 'success', 8500)
			...
			QBCore.Functions.Notify('Du behöver inte en trolle för att gräva upp växten!', 'error', 3500)
		end
        ...
        if DoesEntityExist(Weed) then
            ...
            plant    = GetEntityCoords(entity)                
                if not harvestedPlants[entity] then
                    ...
                    QBCore.Functions.Notify("Plantan har redan skördats...", "error", 3500)
                end
            break
        else
            Wait(1000)
...
function searchPlant(entity)
    ...
    if finished ~= 100 then
		QBCore.Functions.Notify("Det gick inte att skörda plantan...", "error", 3500)
		ClearPedTasks(playerPed)
    else
        ...
        if finished2 ~= 100 then
            ...
            if finished3 ~= 100 then
				...
				QBCore.Functions.Notify("Skördar skörd...", "success", HarvestTime)
                Wait(HarvestTime)
